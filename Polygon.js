class Polygon{
    ...
    updateCenter(){
        ...
        for(let i=0; i<this.vertices.length; i++){
            x = x.add(this.vertices[i]);
        }
        x = x.scale(1/this.vertices.length);
        this.center = x;
    }
    ...
    order(){
         ...
         centralized.sort((a,b)=>{
            let anga = Math.atan2(a.coords[1], a.coords[0]);
            let angb = Math.atan2(b.coords[1], b.coords[0]);
            if(anga<0){
                anga+=2*Math.PI;
            }
            if(angb<0){
    ...
    moveTo(x,y){
         if(!y){
            ...
            var offset = new Vector([x - this.center.coords[0], y - this.center.coords[1]]);
         }
     ...
     static generateConvex(N, bound){
        ...
        for(let i=0; i<xVec.length; i++){
            Vectors.push(new Vector([xVec[i], yVec[i]]));
        }
        ...
        let verts = [Vectors[0].add(new Vector([xmax,ymax]))];
     ...
     * @param {Number} y - y coordinate of rectangle's center
     * @param {Number} width
     * @param {Number} height
     * @returns Rectangle polygon
     */
    static createRect(x,y,width,height){
        return new Polygon([new Vector([x-width/2, y-height/2]), new Vector([x-width/2, y+height/2]), 
                            new Vector([x+width/2, y-height/2]),new Vector([x+width/2, y+height/2])]);
    }
